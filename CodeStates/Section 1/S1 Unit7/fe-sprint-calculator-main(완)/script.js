...
buttons.addEventListener('click', function (event) {
  ...
  if (target.matches('button')) {
    if (action === 'number') {
      ...
      } //처음 입력된 값이 '0'이고 + operatorForAdvanced값이 undefind일 때
      
      else if(display.textContent !== '0' && operatorForAdvanced === undefined){
        display.textContent = display.textContent + buttonContent
        firstNum = display.textContent
      } //처음 입력된 값이 '0'이 아니고+ operatorForAdvanced값이 undefind일 때
      ...
      } //처음 입력된 값이 '0'이 아니고 + operatorForAdvanced값이 undefind가 아닐 때
    }
    if (action === 'operator') {
